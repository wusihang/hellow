    ...
    :QWidget(parent)
{
    ...
    zoomInButton->setIcon(QIcon(":/images/zoomin.jpg"));
    zoomInButton->adjustSize();
    connect(zoomInButton,SIGNAL(clicked()),this,SLOT(zoomIn()));

    zoomOutButton = new QToolButton(this);
    zoomOutButton->setIcon(QIcon(":/images/zoomout.jpg"));
...
void Plotter::mouseMoveEvent(QMouseEvent *event)
{
    if(rubberBandIsShown){
        updateRubberBandRegion();
        //如果鼠标向上移动或者向左移动，setBottomRight将会使得rubberBandRect的长度和高度变为负数
        // 因此在updateRubberBandRegion中需要先对rect进行normalize使得左上角和右下角进行调整
        rubberBandRect.setBottomRight(event->pos());
        updateRubberBandRegion();
    }
}
...
{
    pixmap = QPixmap(size());
   // pixmap.fill(this,0,0); qt4用法，已废弃
...
void Plotter::drawGrid(QPainter *painter)
{
    ...
    if(rect.isValid())
    {
        ...
        for(int i=0;i<=settings.numXTicks;++i)
        {
            ...
            painter->drawText(x-50,rect.bottom()+5,100,20,Qt::AlignCenter|Qt::AlignTop,QString::number(label));
        }
